...
INSERT INTO actors
    WITH params AS (
        SELECT 2021:: int AS y
    ),
    
    last_year AS (
        SELECT *
        FROM actors
        WHERE as_of_year = (SELECT y - 1 FROM params) 
    ),
    ...
    (SELECT y FROM params) AS as_of_year,
    CASE
...
--                 NULL
--             ) AS films,
--             ROW_NUMBER() OVER (PARTITION BY actorid , as_of_year ORDER BY filmid) AS row_num
--         FROM actor_year_films
--     ),

--     windowed_films_deduped AS (
--         SELECT actorid, actor, as_of_year, films
--         FROM windowed_films
--         WHERE row_num = 1
--     ),
