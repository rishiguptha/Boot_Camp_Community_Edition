...
last_year_scd AS (
    ...
    WHERE current_year = (SELECT y - 1 FROM params)
        AND end_year = (SELECT y - 1 FROM params)
),
historical_scd AS (
    SELECT
        actorid,
        ...
        start_year,
        end_year,
        (SELECT y FROM params) AS current_year
    FROM actors_history_scd
    WHERE current_year = (SELECT y - 1 FROM params)
        AND end_year < (SELECT y - 1 FROM params)
),
this_year_actors AS (
    SELECT *
    FROM actors
    WHERE as_of_year = (SELECT y FROM params)
),
